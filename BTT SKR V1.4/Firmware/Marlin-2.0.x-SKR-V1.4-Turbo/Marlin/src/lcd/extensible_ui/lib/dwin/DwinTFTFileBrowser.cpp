...
void DwinTFTFileBrowserClass::listFiles()
{
  ...
  if(strcasecmp(selectedDirectory, PSTR(EXTRA_MENU)) == 0) {
    buildExtraMenu(itemPos);
  } else if(strcasecmp(selectedDirectory, PSTR(DEBUG_MENU)) == 0) {
...
void DwinTFTFileBrowserClass::selectFile()
{
  ...
  if (DwinTFTCommand.TFTstrchr_pointer[lastCharPos] == '/' && 
    ...
    listFiles();
  } else if(strcasecmp(DwinTFTCommand.TFTstrchr_pointer + 4, PSTR(EXTRA_MENU)) == 0 || 
    strcasecmp(DwinTFTCommand.TFTstrchr_pointer + 4, PSTR(DEBUG_MENU)) == 0) {
...
{
  if(strcasecmp(selectedFilename, PSTR(EXTRA_MENU_DIR_UP)) == 0) {
    reset();
    listFiles();
    return;
