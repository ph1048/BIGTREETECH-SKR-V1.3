...
{
  if(msg) {
    if(strcasecmp(msg, PSTR("Reheating...")) == 0) {
...
void DwinTFTClass::onUserConfirmRequired(const char * const msg)
{
    ...
    if(msg) {
      if(strcasecmp(msg, PSTR("HeaterTimeout")) == 0) {
        DWIN_TFT_SERIAL_PROTOCOLPGM(DWIN_TFT_TX_HOTEND_HEATING_START);
        DWIN_TFT_SERIAL_ENTER();
        waitForUserConfirm();
      } else if(strcasecmp(msg, PSTR("Reheat finished.")) == 0) {
        DWIN_TFT_SERIAL_PROTOCOLPGM(DWIN_TFT_TX_HOTEND_HEATING_END);
        DWIN_TFT_SERIAL_ENTER();
        waitForUserConfirm();
      } else if(strcasecmp(msg, PSTR("Load Filament")) == 0) {
        waitForUserConfirm();
      } else if(strcasecmp(msg, PSTR("Filament Purge Running...")) == 0) {
        waitForUserConfirm();
      } else if(strcasecmp(msg, PSTR("Nozzle Parked")) == 0) {
        waitForUserConfirm();
      } else if(strcasecmp(msg, PSTR("MMU2 Eject Recover")) == 0) {
        waitForUserConfirm();
      } else if(strcasecmp(msg, PSTR("M43 Wait Called")) == 0) {
        waitForUserConfirm();
      } else {
